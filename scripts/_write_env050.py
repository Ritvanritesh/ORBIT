"""Write the phase12a_env050.py script."""
from pathlib import Path

script = r'''"""Phase 12A benchmark - ENV-12A-050 with progress."""
...
from orbit.ml.features import build_feature_snapshot, FEATURE_NAMES, attach_decision_times, FeatureSnapshot
...
def run_single(family, params, fs_snap, lab_snap, fs_names, env_id, fs_id):
    ...
    try:
        ...
        try:
            ...
            m["oos_ic_sessions"] = r.get("sessions_used", 0) if isinstance(r, dict) else 0
        except Exception:
            m["oos_ic"] = None
        try:
...
def main():
    ...
    persist_phase12a_plan(plan)
    p(f"Plan: {plan['n_experiments']} experiments, digest {plan['plan_digest'][:16]}...")
    ...
    t0 = time.time(); p("  Market...", end=" "); mf = compute_market_features(benchmark_bars, universe); p(f"{mf.height} rows ({time.time()-t0:.1f}s)")
    t0 = time.time(); p("  Sector...", end=" "); sf = compute_sector_features(bars, sector_map, universe); p(f"{sf.height} rows ({time.time()-t0:.1f}s)")
    t0 = time.time(); p("  Cross-sectional...", end=" "); xf = compute_cross_sectional_features(fs001.records, universe, FEATURE_NAMES); p(f"{xf.height} rows ({time.time()-t0:.1f}s)")
    ...
    for fam, par in MODELS:
        for fs_id in fsets:
            ...
            fnames = [PHASE12A_FEATURE_NAMES.get(f,f) for f in snap.feature_refs]
            elapsed = time.time()-t_start
            eta = (elapsed/max(done-1,1))*(total-done) if done > 1 else 0
            p(f"  [{done}/{total}] {fam}+{fs_id} ({elapsed:.0f}s, ETA {eta:.0f}s)", end=" ")
...
Path("scripts/phase12a_env050.py").write_text(script, encoding="utf-8")
