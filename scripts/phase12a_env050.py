"""Phase 12A benchmark - ENV-12A-050 with progress."""
...
def get_feature_names(snap):
    """Convert feature_refs (IDs) to column names."""
...
def main():
    ...
    t0 = time.time()
    print("  Market...", end=" ", flush=True)
    mf = compute_market_features(benchmark_bars, universe)
    print(f"{mf.height} rows ({time.time()-t0:.1f}s)")
    t0 = time.time()
    print("  Sector...", end=" ", flush=True)
    sf = compute_sector_features(bars, sector_map, universe)
    print(f"{sf.height} rows ({time.time()-t0:.1f}s)")
    t0 = time.time()
    print("  Cross-sectional...", end=" ", flush=True)
    xf = compute_cross_sectional_features(fs001.records, universe, FEATURE_NAMES)
    print(f"{xf.height} rows ({time.time()-t0:.1f}s)")
