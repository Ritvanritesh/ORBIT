"""Phase 12E implementation: corrected LAB-006 excess-return revalidation."""
...
def compute_excess_labels(inst_bars, bench_bars, decisions, horizon=5):
    ...
    df = pl.DataFrame(results, schema={
        ...
        "instrument_return": pl.Float64,
        "benchmark_return": pl.Float64,
        "label_available": pl.Boolean,
        "outcome_status": pl.Utf8,
        "unavailable_reason": pl.Utf8,
    })
    # Add decision_session (date) for join compatibility
    df = df.with_columns(
        pl.col("decision_time").dt.date().alias("decision_session")
    )
    return df
...
def assemble_ds(feat_records, lab_records, feat_names):
    ...
    result = assemble_datasets(_S(feat_records), _S(lab_records), feature_names=feat_names)
    return result
...
def train_eval(family, params, feat_names, ds, seed=42):
    ...
    model, state = train_model(family, params, Xt, yt, feature_names=feat_names)
    pred_val = predict_with_state(model, state, Xv)
    pred_test = predict_with_state(model, state, Xs)
    try:
