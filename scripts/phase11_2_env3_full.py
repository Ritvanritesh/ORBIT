"""Run full ENV-3 benchmark with correct locked params."""
...
print(f"DS-EXP-050: {n_inst} instruments, {bars['trade_date'].n_unique()} sessions")

fs001_snap = build_feature_snapshot(bars, data_refs=["DS-EXP-050"])
...
fs003_snap = FeatureSnapshot(
    ...
    feature_refs=fs003_names, data_refs=["DS-EXP-050"], records=fs003_frame,
)
...
for i, (family, params) in enumerate(LOCKED_MODELS):
    for fs_id, fs_snap, fs_names in feature_sets:
        ...
        result = run_experiment(family, params, fs_snap, lab, fs_names, "ENV-3", fs_id)
...
persist_results(res_env3)
with open("benchmarks/phase11_2_ENV-3_results.json", "w") as f:
    json.dump(res_env3, f, indent=2, default=str)
print("ENV-3 saved")
